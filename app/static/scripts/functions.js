...
(function($){
	...
	SEMICOLON.initialize = {
		...
		resizeVideos: function(){
			...
			$("#css,#footer,.slider-element:not(.revslider-wrap),.landing-offer-media,.portfolio-ajax-modal,.mega-menu-column").fitVids({
	...
	SEMICOLON.header = {
		...
		superfish: function(){
			...
			if( $body.hasClass('device-xl') || $body.hasClass('device-lg') ) {
				$('#primary-menu ul ul, #primary-menu ul .mega-menu-css').css('display', 'block');
				SEMICOLON.header.menuInvert();
				$('#primary-menu ul ul, #primary-menu ul .mega-menu-css').css('display', '');
				...
				$('body:not(.side-header) #primary-menu:not(.on-click) > ul, body:not(.side-header) #primary-menu:not(.on-click) > div > ul:not(.dropdown-menu), .top-links:not(.on-click) > ul').superfish({
					popUpSelector: 'ul,.mega-menu-css,.top-link-section',
					delay: 250,
					speed: 350,
		...
		menuInvert: function() {
			...
			$('#primary-menu .mega-menu-css, #primary-menu ul ul').each( function( index, element ){
		...
		menufunctions: function(){
			...
			if( $('#primary-menu').hasClass('on-click') || ( $body.hasClass('device-md') || $body.hasClass('device-sm') || $body.hasClass('device-xs') ) ) {
				$('#primary-menu li:has(ul) > a').on( 'click touchend', function(e){
					$(this).parents('.sub-menu').siblings().find('ul,.mega-menu-css').removeClass('d-block');
					$(this).parent('li').children('ul,.mega-menu-css').toggleClass('d-block');
					e.preventDefault();
				});
			}
		...
		fullWidthMenu: function(){
			if( $body.hasClass('stretched') ) {
				if( $header.find('.container-fullwidth').length > 0 ) { $('.mega-menu .mega-menu-css').css({ 'width': $wrapper.width() - 120 }); }
				if( $header.hasClass('full-header') ) { $('.mega-menu .mega-menu-css').css({ 'width': $wrapper.width() - 60 }); }
			} else {
				if( $header.find('.container-fullwidth').length > 0 ) { $('.mega-menu .mega-menu-css').css({ 'width': $wrapper.width() - 120 }); }
				if( $header.hasClass('full-header') ) { $('.mega-menu .mega-menu-css').css({ 'width': $wrapper.width() - 80 }); }
			}
		},
			...
			if( $tabs.length > 0 ) {
				$tabs.each( function(){
					...
					if( jQuery(windowHash).length > 0 ) {
						...
						tabActive = jQuery( ".tab-css" ).index( tabItem );
					}
		...
		tabsResponsiveResize: function(){
			...
			$tabsResponsive.each( function(){
				...
				if( $('body').hasClass('device-sm') || $('body').hasClass('device-xs') ) {
					...
					element.find('.tab-css').addClass('acc_content');
					...
					element.find('.tab-css').removeClass('acc_content');
